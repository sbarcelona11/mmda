...
/**
 * Provides a form for deleting a Product entity revision.
 *
 * @ingroup product
 */
class ProductEntityRevisionDeleteForm extends ConfirmFormBase {
  ...
  public static function create(ContainerInterface $container) {
    ...
    return new static(
      $entity_type_manager->getStorage('product_entity'),
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Product entity: deleted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Revision from %revision-date of Product entity %title has been deleted.', ['%revision-date' => format_date($this->revision->getRevisionCreationTime()), '%title' => $this->revision->label()]));
